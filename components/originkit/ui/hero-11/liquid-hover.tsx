...
export default function LiquidHover({
  ...
  useEffect(() => {
    ...
    const canUseHalfFloat = Boolean(halfFloat && halfFloatColor);
    const canUseFloat = Boolean(floatTexture && floatColor);
    const simulationTextureType = canUseHalfFloat
      ? halfFloat.HALF_FLOAT_OES
      : canUseFloat
        ? gl.FLOAT
        : gl.UNSIGNED_BYTE;
    const simulationTextureFilter =
      (canUseHalfFloat && halfFloatLinear) || (canUseFloat && floatLinear)
        ? gl.LINEAR
        : gl.NEAREST;
...
`;
    ...
    function setupEvents() {
      const supportsMouseHover = window.matchMedia(
      ...
      const onPointerTouch = (e: PointerEvent) => {
        if (e.pointerType === "mouse") return;
        isHovering = true;
        const rect = container!.getBoundingClientRect();
        updatePointerPosition(e.clientX - rect.left, e.clientY - rect.top);
      };
      const onPointerTouchEnd = (e: PointerEvent) => {
        if (e.pointerType === "mouse") return;
        isHovering = false;
      };
      const onResize = () => {
      ...
      container!.addEventListener("pointerdown", onPointerTouch, { passive: true });
      container!.addEventListener("pointermove", onPointerTouch, { passive: true });
      container!.addEventListener("pointerup", onPointerTouchEnd, { passive: true });
      container!.addEventListener("pointercancel", onPointerTouchEnd, {
        passive: true,
      });
      ...
      return () => {
        ...
        container!.removeEventListener("pointerdown", onPointerTouch);
        container!.removeEventListener("pointermove", onPointerTouch);
        container!.removeEventListener("pointerup", onPointerTouchEnd);
        container!.removeEventListener("pointercancel", onPointerTouchEnd);
